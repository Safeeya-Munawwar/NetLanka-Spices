...
export default function OrderConfirmation() {
  const { cartItems, clearCart, updateQuantity, removeFromCart, addToCart } =
    useCart();
  const { user } = useUser();
  ...
  const calculateTotalPriceLKR = () =>
    cartItems.reduce(
      (sum, item) =>
        sum + (item.priceLKR || 0) * parseWeight(item.weight) * item.quantity,
      0
    );
  ...
  const calculateTotalPriceUSD = () =>
    cartItems.reduce(
      (sum, item) =>
        sum + (item.priceUSD || 0) * parseWeight(item.weight) * item.quantity,
      0
    );
  ...
  const handleQuantityChange = (productId, newQty) => {
    ...
    setTimeout(
      () => setAnimateBadge((prev) => ({ ...prev, [productId]: false })),
      300
    );
  };
  ...
  if (confirmed) {
    return (
      <div className="container mx-auto py-16 text-center">
        ...
        <p className="text-[#3D2B1F] mb-6">
          Total Paid: LKR {calculateTotalPriceLKR().toLocaleString()}
        </p>
        <a
          href="/products"
    ...
    <div className="container mx-auto py-16 px-4 md:px-0">
      {!cartItems.length ? (
        <div className="text-center mb-8">
          ...
          <a
            href="/products"
            className="text-[#A65B05] font-bold px-4 py-2 border-2 border-[#A65B05] rounded-lg hover:bg-[#A65B05] hover:text-white transition"
          >
            View Products
          </a>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 mt-8">
              ...
              >
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <p className="text-[#3D2B1F]">
                    Weight: {parseWeight(item.weight)} kg
                  </p>
                  <p className="text-[#3D2B1F]">
                    Price: LKR{" "}
                    {(
                      item.priceLKR * parseWeight(item.weight)
                    ).toLocaleString()}{" "}
                    / ${(item.priceUSD * parseWeight(item.weight)).toFixed(2)}
                  </p>
          ...
          <div className="lg:w-1/3 p-6 border rounded-lg shadow bg-yellow-50 sticky top-20 h-fit flex flex-col gap-4">
            ...
            <div className="flex justify-between font-bold text-[#5C4033] text-lg mb-4 border-t pt-2">
              <span>Total:</span>
              <span>
                LKR {calculateTotalPriceLKR().toLocaleString()} / $
                {calculateTotalPriceUSD().toFixed(2)}
              </span>
            </div>
            ...
            >
              ...
              <div className="mb-2">
                {order.items.map((item) => (
                  <div
                    key={item.productId + item.weightKg}
                    className="flex justify-between text-sm py-1 border-b last:border-b-0"
                  >
                    <span>
                      {item.name} ({item.weightKg ?? 1} kg)
                    </span>
                    <span>
                      LKR {(item.priceLKR ?? 0).toLocaleString()} / $
                      {(item.priceUSD ?? 0).toFixed(2)} × {item.quantity} = LKR{" "}
                      {((item.priceLKR ?? 0) * item.quantity).toLocaleString()}{" "}
              ...
              <div className="text-right font-bold text-[#5C4033]">
                Total: LKR {(order.totalPriceLKR ?? 0).toLocaleString()} / $
                {(order.totalPriceUSD ?? 0).toFixed(2)}
              </div>
            </div>
          ))
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {recommendedProducts.map((product) => (
            ...
            >
              ...
              <button
                className="w-full py-1 bg-yellow-900 text-white rounded hover:bg-yellow-800 transition"
                onClick={() =>
                  addToCart({
                    ...product,
                    weight: selectedWeights[product.id],
                    quantity: 1,
                  })
                }
              >
