...
export default function CartSidebar({
  ...
  const handleQuantityChange = (id, delta) => {
    ...
    if (newQuantity <= 0) removeFromCart(id);
      ...
      setTimeout(
        () => setAnimateBadge((prev) => ({ ...prev, [id]: false })),
        300
      );
    }
  };
  ...
  return (
    <>
      ...
      <button
        ...
        className="
    fixed bottom-20 right-6   /* move cart below WhatsApp */
  ...
  "
    ...
    "
      ...
      <div
        className={`
        fixed top-0 right-0 h-full z-50 bg-yellow-50 transform transition-transform duration-300 shadow-2xl flex flex-col
        ${open ? "translate-x-0" : "translate-x-full"} 
        w-full sm:w-[28rem] md:w-[35rem]
      `}
      >
        {/* Header */}
        ...
        <div className="flex-grow overflow-y-auto p-4">
          {cartItems.length === 0 ? (
            <p className="text-yellow-800 text-center mt-10">
              Your cart is empty.
            </p>
          ) : (
        ...
        {cartItems.length > 0 && (
          <div className="p-6 border-t border-yellow-200 flex flex-col gap-3">
            <p className="text-yellow-900 font-bold text-lg">
              Total: LKR {totalPriceLKR.toLocaleString()} / ${" "}
              {totalPriceUSD.toFixed(2)}
            </p>
            <button
              className="w-full bg-yellow-900 text-white p-3 rounded font-semibold hover:bg-yellow-800 transition"
              onClick={() => navigate("/order-confirmation")}
            >
              Checkout
            </button>
            <button
              className="w-full text-red-600 font-semibold hover:text-red-800 mt-2"
              onClick={clearCart}
            >
              Clear Cart
            </button>
